...
def test_controller_identity_oidc_and_workflow_are_closed_and_separate() -> None:
    ...
    assert "param presentationImageRegistryRoleAssignmentMode string" in orchestration
    assert (
    ...
    example_parameters = _read("infra/wc013-live-acceptance/main.example.bicepparam")
    assert (
        "param acceptanceImageRegistryRoleAssignmentMode = 'LegacyRegistryPermissions'"
    ) in example_parameters
    assert (
    ...
    deployment_name = "wc013-ready-20260903T025500Z-c9a420be0cbb"
    ...
    assert reviewed_index["deployments"][deployment_name] == {
        ...
        "deploymentCorrelationId": "6d1a2892-6384-46a8-9a78-d18f647d54d4",
        "deploymentTemplateHash": "8373313941976686749",
        "sourceCommit": "c9a420be0cbbf2596f179c5e80dbb164a0637609",
    }
...
def test_confirmed_parameters_use_published_images() -> None:
    ...
    assert parameters["acceptanceImage"]["value"].endswith(
        "@sha256:fb9503b3a9db31a518f5d18c1e3a27ee6f203e1e84d35640ff8b58f5438720a1"
    )
    assert parameters["presentationImage"]["value"] == (
        "athenademoa6add389.azurecr.io/athena/presentation-web"
        "@sha256:e0116228ea3e6e17d382de2bd1172481e876de525f4c1fcd16466ef267bf7146"
    )
    orchestration = _read("infra/wc013-live-acceptance/main.bicep")
    presentation = _read(
        "infra/wc013-live-acceptance/modules/presentation-web.bicep"
