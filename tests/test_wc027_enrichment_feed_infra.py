...
def test_wc027_readiness_rejects_missing_or_mismatched_publisher_and_bindings() -> None:
    ...
    # An explicit binding-publisher gate, false by default.
    assert "param wc027PublisherReady bool = false" in source
    publisher_parameter = source[
        source.index("@description('Explicit confirmation that the separately governed")
        : source.index(
            "@description('Confirms the exact legacy WC-016 resources",
        )
    ]
    assert "@allowed([\n  false\n])" in publisher_parameter
    ...
    assert "wc027FeedV2ProducerReady && !wc027PublisherReady" in source
