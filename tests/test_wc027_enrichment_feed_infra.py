...
def test_wc027_notification_gate_requires_deployed_job_resource_id() -> None:
    ...
    assert "wc027ProducerJob!.tags.monitoringCollectorContractDigest" in source
    assert "wc027ProducerJob!.tags.monitoringAcquisitionAuthorityDigest" in source
    assert "wc027MonitoringCollectorContractDigestValid" in source
    assert "wc027MonitoringAcquisitionAuthorityDigestValid" in source
    assert "wc027ProducerMonitoringTrustIsCurrent" in source
    assert "ATHENA_WC027_ENRICHMENT_FEED_CONFIG_JSON" in source
...
def test_wc027_readiness_rejects_missing_or_mismatched_publisher_and_bindings() -> None:
    ...
    assert "wc027PublisherMonitoringTrustMatches" in source
    assert "wc027PublisherJob!.tags.monitoringCollectorContractDigest" in source
    assert "wc027PublisherJob!.tags.monitoringAcquisitionAuthorityDigest" in source
