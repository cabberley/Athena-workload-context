...
module inventoryAttestationValidation 'modules/monitoring-rbac-inventory-attestation-validation.bicep' = {
  ...
  params: {
    attestationSchemaVersion: inventoryAttestation.schemaVersion
    ...
    runtimeSupportIdentityPrincipalId: contractInputs.runtimeSupportIdentityPrincipalId
...
var targetReadQueryMode = 'assignedToPrincipalIncludingInheritedGroupsAndDescendants'
var collectorTargetReadEvidence = collectorEvidence.targetReadEvidence
var contextTargetReadEvidence = contextEvidence.targetReadEvidence
var runtimeSupportTargetReadEvidence = runtimeSupportEvidence.targetReadEvidence
var normalizedCollectorTargetScopeIds = map(
  collectorTargetReadEvidence,
  targetRead => toLower(targetRead.targetScopeId)
)
var normalizedContextTargetScopeIds = map(
  contextTargetReadEvidence,
  targetRead => toLower(targetRead.targetScopeId)
)
var normalizedRuntimeSupportTargetScopeIds = map(
  runtimeSupportTargetReadEvidence,
  targetRead => toLower(targetRead.targetScopeId)
)
...
var collectorTargetDigests = map(collectorTargetReadEvidence, targetRead => targetRead.targetDigest)
var contextTargetDigests = map(contextTargetReadEvidence, targetRead => targetRead.targetDigest)
var runtimeSupportTargetDigests = map(runtimeSupportTargetReadEvidence, targetRead => targetRead.targetDigest)
var collectorTargetPages = flatten(map(collectorTargetReadEvidence, targetRead => targetRead.rawPageDigests))
var contextTargetPages = flatten(map(contextTargetReadEvidence, targetRead => targetRead.rawPageDigests))
var runtimeSupportTargetPages = flatten(map(runtimeSupportTargetReadEvidence, targetRead => targetRead.rawPageDigests))
var collectorTargetBindingsAreValid = empty(filter(
  collectorTargetReadEvidence,
  targetRead => targetRead.queryMode != targetReadQueryMode || targetRead.readCount != 2 || targetRead.allPagesRetrieved != true || empty(targetRead.rawPageDigests) || length(union(targetRead.rawPageDigests, [])) != length(targetRead.rawPageDigests) || targetRead.bindingId != guid(toLower(collectorEvidence.principalId), toLower(targetRead.targetScopeId), targetRead.queryMode, targetRead.targetDigest, join(targetRead.rawPageDigests, ','), string(targetRead.readCount), string(targetRead.allPagesRetrieved))
))
var contextTargetBindingsAreValid = empty(filter(
  contextTargetReadEvidence,
  targetRead => targetRead.queryMode != targetReadQueryMode || targetRead.readCount != 2 || targetRead.allPagesRetrieved != true || empty(targetRead.rawPageDigests) || length(union(targetRead.rawPageDigests, [])) != length(targetRead.rawPageDigests) || targetRead.bindingId != guid(toLower(contextEvidence.principalId), toLower(targetRead.targetScopeId), targetRead.queryMode, targetRead.targetDigest, join(targetRead.rawPageDigests, ','), string(targetRead.readCount), string(targetRead.allPagesRetrieved))
))
var runtimeSupportTargetBindingsAreValid = empty(filter(
  runtimeSupportTargetReadEvidence,
  targetRead => targetRead.queryMode != targetReadQueryMode || targetRead.readCount != 2 || targetRead.allPagesRetrieved != true || empty(targetRead.rawPageDigests) || length(union(targetRead.rawPageDigests, [])) != length(targetRead.rawPageDigests) || targetRead.bindingId != guid(toLower(runtimeSupportEvidence.principalId), toLower(targetRead.targetScopeId), targetRead.queryMode, targetRead.targetDigest, join(targetRead.rawPageDigests, ','), string(targetRead.readCount), string(targetRead.allPagesRetrieved))
))
...
var collectorTargetsMatch = collectorTargetsAreUnique && collectorTargetBindingsAreValid && length(union(collectorTargetDigests, [])) == length(collectorTargetDigests) && length(union(collectorTargetPages, [])) == length(collectorTargetPages) && length(union(map(collectorTargetReadEvidence, targetRead => targetRead.bindingId), [])) == length(collectorTargetReadEvidence) && length(normalizedCollectorTargetScopeIds) == length(expectedPrincipalTargetScopeIds) && length(union(normalizedCollectorTargetScopeIds, expectedPrincipalTargetScopeIds)) == length(expectedPrincipalTargetScopeIds)
var contextTargetsMatch = contextTargetsAreUnique && contextTargetBindingsAreValid && length(union(contextTargetDigests, [])) == length(contextTargetDigests) && length(union(contextTargetPages, [])) == length(contextTargetPages) && length(union(map(contextTargetReadEvidence, targetRead => targetRead.bindingId), [])) == length(contextTargetReadEvidence) && length(normalizedContextTargetScopeIds) == length(expectedPrincipalTargetScopeIds) && length(union(normalizedContextTargetScopeIds, expectedPrincipalTargetScopeIds)) == length(expectedPrincipalTargetScopeIds)
var runtimeSupportTargetsMatch = runtimeSupportTargetsAreUnique && runtimeSupportTargetBindingsAreValid && length(union(runtimeSupportTargetDigests, [])) == length(runtimeSupportTargetDigests) && length(union(runtimeSupportTargetPages, [])) == length(runtimeSupportTargetPages) && length(union(map(runtimeSupportTargetReadEvidence, targetRead => targetRead.bindingId), [])) == length(runtimeSupportTargetReadEvidence) && length(normalizedRuntimeSupportTargetScopeIds) == length(expectedPrincipalTargetScopeIds) && length(union(normalizedRuntimeSupportTargetScopeIds, expectedPrincipalTargetScopeIds)) == length(expectedPrincipalTargetScopeIds)
...
var principalEvidenceIsComplete = collectorEvidence.allPagesRetrieved == true && contextEvidence.allPagesRetrieved == true && runtimeSupportEvidence.allPagesRetrieved == true && empty(filter([
  collectorEvidence
  contextEvidence
  runtimeSupportEvidence
], evidence => contains(evidence, 'queryFilter') || contains(evidence, 'includeInherited') || contains(evidence, 'includeGroups') || contains(evidence, 'includeAllDescendantScopes') || contains(evidence, 'targetScopeIds') || contains(evidence, 'firstReadTargetDigests') || contains(evidence, 'secondReadTargetDigests') || contains(evidence, 'roleAssignmentRawPageDigests') || contains(evidence, 'transitiveGroupRawPageDigests') || contains(evidence, 'firstRoleAssignmentRawPageDigests') || contains(evidence, 'secondRoleAssignmentRawPageDigests'))) && string(collectorEvidence.transitiveGroupIds) == string(inventory.collectorSecurityGroupIds) && string(contextEvidence.transitiveGroupIds) == string(inventory.athenaContextSecurityGroupIds) && empty(runtimeSupportEvidence.transitiveGroupIds) && !empty(collectorEvidence.firstTransitiveGroupRawPageDigests) && string(collectorEvidence.firstTransitiveGroupRawPageDigests) == string(collectorEvidence.secondTransitiveGroupRawPageDigests) && !empty(contextEvidence.firstTransitiveGroupRawPageDigests) && string(contextEvidence.firstTransitiveGroupRawPageDigests) == string(contextEvidence.secondTransitiveGroupRawPageDigests) && !empty(runtimeSupportEvidence.firstTransitiveGroupRawPageDigests) && string(runtimeSupportEvidence.firstTransitiveGroupRawPageDigests) == string(runtimeSupportEvidence.secondTransitiveGroupRawPageDigests)
...
var expectedResourceGraphQueryRoleActions = [
  toLower(contractInputs.resourceHealthAllowedOperations[0])
]
var expectedResourceHealthRoleActions = [
  toLower(contractInputs.resourceHealthAllowedOperations[1])
]
var resourceHealthOperationTupleIsExact = contractInputs.resourceHealthAllowedOperations == [
  'Microsoft.ResourceGraph/resources/read'
  'Microsoft.ResourceHealth/availabilityStatuses/read'
]
var resourceHealthReasonAuthorityIsExact = contractInputs.resourceHealthReasonAuthorityMode == 'availabilityStatusUnknownOnly' && contractInputs.resourceHealthReasonEvidenceVersion == 2
...
var expectedCustomRoleDefinitions = [
  ...
  {
    roleDefinitionId: toLower(contractInputs.resourceGraphQueryRoleDefinitionId)
    actions: expectedResourceGraphQueryRoleActions
  }
  {
    roleDefinitionId: toLower(contractInputs.resourceHealthRoleDefinitionId)
    actions: expectedResourceHealthRoleActions
  }
  {
...
var verifierTargetReadEvidence = verifierPrincipalEvidence.targetReadEvidence
var verifierTargetRead = verifierTargetReadEvidence[0]
var verifierPrincipalEvidenceMatches = toLower(verifierPrincipalEvidence.principalId) == toLower(contractInputs.rbacInventoryVerifierIdentityPrincipalId) && length(verifierTargetReadEvidence) == 1 && toLower(verifierTargetRead.targetScopeId) == toLower(subscriptionScope) && verifierTargetRead.queryMode == targetReadQueryMode && verifierTargetRead.readCount == 2 && verifierTargetRead.allPagesRetrieved == true && !empty(verifierTargetRead.rawPageDigests) && length(union(verifierTargetRead.rawPageDigests, [])) == length(verifierTargetRead.rawPageDigests) && verifierTargetRead.bindingId == guid(toLower(verifierPrincipalEvidence.principalId), toLower(verifierTargetRead.targetScopeId), verifierTargetRead.queryMode, verifierTargetRead.targetDigest, join(verifierTargetRead.rawPageDigests, ','), string(verifierTargetRead.readCount), string(verifierTargetRead.allPagesRetrieved)) && empty(verifierPrincipalEvidence.transitiveGroupIds) && verifierPrincipalEvidence.allPagesRetrieved == true && !contains(verifierPrincipalEvidence, 'queryFilter') && !contains(verifierPrincipalEvidence, 'includeInherited') && !contains(verifierPrincipalEvidence, 'includeGroups') && !contains(verifierPrincipalEvidence, 'includeAllDescendantScopes') && !contains(verifierPrincipalEvidence, 'targetScopeIds') && !contains(verifierPrincipalEvidence, 'firstReadTargetDigests') && !contains(verifierPrincipalEvidence, 'secondReadTargetDigests') && !contains(verifierPrincipalEvidence, 'roleAssignmentRawPageDigests') && !contains(verifierPrincipalEvidence, 'transitiveGroupRawPageDigests') && !contains(verifierPrincipalEvidence, 'firstRoleAssignmentRawPageDigests') && !contains(verifierPrincipalEvidence, 'secondRoleAssignmentRawPageDigests') && !empty(verifierPrincipalEvidence.firstTransitiveGroupRawPageDigests) && string(verifierPrincipalEvidence.firstTransitiveGroupRawPageDigests) == string(verifierPrincipalEvidence.secondTransitiveGroupRawPageDigests)
...
var resourceHealthAuthorizationSummaryMatches = inventory.resourceGraphQueryRoleActions == expectedResourceGraphQueryRoleActions && inventory.resourceHealthRoleActions == expectedResourceHealthRoleActions
var resourceGraphQueryScopeIsExact = normalizedResourceGraphQueryScopeId == toLower(contractInputs.workloadResourceGroupId)

var inventoryCoreIsValid = inventory.schemaVersion == 'athena.wc028MonitoringEffectiveRbacInventory.v6' && inventory.scopeCollectionMode == 'subscriptionAssignedToAllInheritedAndUnfilteredWithProtectedScopes' && inventory.ancestorScopeCollectionComplete == true && inventory.subscriptionDescendantCollectionComplete == true && managementGroupAncestryIsComplete && hierarchyEvidenceIsComplete && inventoryIsFresh && inventorySourceMatches && resourceHealthOperationTupleIsExact && resourceHealthReasonAuthorityIsExact && resourceHealthAuthorizationSummaryMatches && resourceGraphQueryScopeIsExact && toLower(inventory.subscriptionId) == toLower(handoff.subscriptionId) && toLower(inventory.tenantId) == toLower(handoff.tenantId) && toLower(inventory.collectorPrincipalId) == toLower(handoff.monitoringReaderPrincipalId) && toLower(inventory.athenaContextPrincipalId) == toLower(handoff.athenaContextPrincipalId) && toLower(collectorEvidence.principalId) == toLower(inventory.collectorPrincipalId) && toLower(contextEvidence.principalId) == toLower(inventory.athenaContextPrincipalId) && toLower(inventory.attestorIdentityResourceId) == toLower(contractInputs.rbacAttestorIdentityResourceId) && toLower(inventory.attestorClientId) == toLower(contractInputs.rbacAttestorIdentityClientId) && toLower(inventory.attestorPrincipalId) == toLower(contractInputs.rbacAttestorPrincipalId) && toLower(inventory.attestorTenantId) == toLower(contractInputs.rbacAttestorTenantId) && collectorTargetsMatch && contextTargetsMatch && runtimeSupportTargetsMatch
...
module inventoryCoreValidationGate 'modules/monitoring-publication-validation-gate.bicep' = {
  ...
  params: {
    ...
    failureMessage: 'monitoring contract publication requires a fresh v6 inventory with target-bound ancestor reads, workload-RG Resource Graph query, and per-VM Resource Health authorization evidence'
  }
}
...
module collectorContract 'modules/monitoring-collector-contract.bicep' = {
  ...
  params: {
    ...
    resourceHealthReasonAuthorityMode: contractInputs.resourceHealthReasonAuthorityMode
    resourceHealthReasonEvidenceVersion: contractInputs.resourceHealthReasonEvidenceVersion
