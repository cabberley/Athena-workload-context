...
resource signerRole 'Microsoft.Authorization/roleDefinitions@2022-04-01' = {
  name: guid(key.id, 'athena-wc027-key-sign-verify')
  properties: {
    roleName: 'Athena WC027 Exact Key Sign and Verify (${keyVaultName}/${keyName})'
    description: 'Sign and verify with this exact WC-027 key. No key read, encrypt, decrypt, wrap, unwrap, release, backup, restore, update, or delete.'
    type: 'CustomRole'
    assignableScopes: [
      resourceGroup().id
    ]
...
resource signer 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(key.id, identityResourceId, signerRole.id)
  scope: key
  properties: {
