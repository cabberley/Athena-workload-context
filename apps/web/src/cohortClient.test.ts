...
import { proposalReviewCandidate } from './cohortDraft'
...
const decisionLoadRequest: CohortDecisionLoadRequest = {
  ...loadRequest,
  scope,
...
const wireDecision = {
  decisionId: 'decision-wc023-synthetic',
  action: 'approve',
  sourceDraft,
  scope,
  proposalIds: [proposal.proposalId],
  proposalSetDigest: wireBatch.proposalSetDigest,
  snapshotArtifactDigest: snapshot.artifactDigest,
  candidateId: 'candidate-wc023-approve',
  rationale: 'Human accepted the exact bounded synthetic selector.',
  state: 'applied',
  decidedBy: mockAuthSession.actorId,
  decidedAt: '2026-08-17T00:03:00.000Z',
  draftResult: {
...
describe('typed cohort proposal HTTP adapter', () => {
  ...
  it('loads exact durable decisions through the merged decision route', async () => {
    const fetchMock = vi.fn(async (...request: Parameters<typeof fetch>) => {
      void request
    ...
    const decisions = await client.loadDecisions(decisionLoadRequest)

    expect(decisions).toEqual([wireDecision])
    const [url, init] = fetchMock.mock.calls[0]!
    expect(String(url)).toContain('/v1/cohort-proposals/decisions?')
    expect(String(url)).toContain(`proposal_ids=${proposal.proposalId}`)
  ...
  it('submits an exact durable decision without a WC-007 draft write', async () => {
    ...
    const candidate = proposalReviewCandidate(
      parsedBatch.proposals[0]!,
      parsedBatch,
      wireDecision.rationale,
    )
    ...
    const fetchMock = vi.fn(async (...request: Parameters<typeof fetch>) => {
      void request
      return response(submittedDecision, 201)
    })
    const client = createCohortProposalApiClient({
      baseUrl: 'https://cohorts.invalid',
      authPort,
    ...
    const decision = await client.submitDecision({
      ...
      action: 'approve',
      candidate,
      rationale: wireDecision.rationale,
    })

    expect(decision).toEqual(submittedDecision)
    const [url, init] = fetchMock.mock.calls[0]!
    expect(url).toBe('https://cohorts.invalid/v1/cohort-proposals/decisions')
    expect(init?.method).toBe('POST')
    expect(String(init?.body)).not.toContain('replacement_manifest')
    expect(new Headers(init?.headers).get('Idempotency-Key')).toMatch(
      /^cohort-decision-[a-f0-9]{32}$/,
    )
  })
})
