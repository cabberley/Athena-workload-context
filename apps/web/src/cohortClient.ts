...
const parseCohortDecisionRecord = (value: unknown): CohortDecisionRecord => {
  const record = asRecord(value, 'cohort decision')
  const action = requiredString(record, 'action', 'cohort decision', 16)
  const state = requiredString(record, 'state', 'cohort decision', 16)
  if (!['approve', 'reject', 'split', 'merge'].includes(action)) {
    throw new Error('Cohort API returned an invalid cohort decision action.')
  }
  if (!['applied', 'rejected'].includes(state)) {
    throw new Error('Cohort API returned an invalid cohort decision state.')
  }
  const proposalIds = stringArray(
    record.proposalIds,
    'cohort decision.proposalIds',
    200,
    25,
  )
  uniqueNormalized(proposalIds, 'cohort decision proposal IDs')
  const candidateId = nullableString(record, 'candidateId', 'cohort decision', 128)
  const draftResultValue = record.draftResult
  const draftResult = draftResultValue === null
    ? null
    : (() => {
        const result = asRecord(draftResultValue, 'cohort decision.draftResult')
        return {
          draftId: requiredString(result, 'draftId', 'cohort decision.draftResult', 128),
          revision: requiredInteger(
            result,
            'revision',
            'cohort decision.draftResult',
            1,
  ...
  const rejected = action === 'reject' && state === 'rejected'
  const applied = action !== 'reject' && state === 'applied'
  if (
    (!rejected && !applied) ||
    (rejected && (candidateId !== null || draftResult !== null)) ||
    (applied && (candidateId === null || draftResult === null)) ||
    requiredBoolean(record, 'publicationAllowed', 'cohort decision') !== false
  ) {
    throw new Error('Cohort API returned an inconsistent cohort decision outcome.')
  }
  const decisionId = requiredString(record, 'decisionId', 'cohort decision', 128)
  const decidedBy = requiredString(record, 'decidedBy', 'cohort decision', 128)
  if (!IDENTIFIER.test(decisionId) || !IDENTIFIER.test(decidedBy)) {
    throw new Error('Cohort API returned invalid cohort decision identifiers.')
  }
  return {
    decisionId,
    action: action as CohortDecisionRecord['action'],
    sourceDraft: parseDraftBinding(record.sourceDraft, 'cohort decision.sourceDraft'),
    scope: parseScope(record.scope, 'cohort decision.scope'),
    proposalIds,
    proposalSetDigest: digest(record, 'proposalSetDigest', 'cohort decision'),
    ...
    candidateId,
    rationale: requiredString(record, 'rationale', 'cohort decision', 2000),
    state: state as CohortDecisionRecord['state'],
    decidedBy,
    decidedAt: timestamp(record, 'decidedAt', 'cohort decision'),
    draftResult,
    publicationAllowed: false,
  }
}
...
const assertDecisionBinding = (
  ...
  if (
    ...
    decision.sourceDraft.revision !== request.sourceDraft.revision ||
    decision.sourceDraft.manifestDigest !== request.sourceDraft.manifestDigest ||
    decision.scope.manifestId !== request.scope.manifestId ||
    decision.scope.manifestVersion !== request.scope.manifestVersion ||
    decision.scope.profileId !== request.scope.profileId ||
    decision.scope.profileType !== request.scope.profileType ||
...
export const createCohortProposalApiClient = (
...
): CohortProposalApiPort & CohortDecisionApiPort => {
  ...
  return {
    ...
    loadDecisions: async (request) => {
      assertAuthorized(request.workloadId)
      uniqueNormalized(request.proposalIds, 'decision request proposal IDs')
      const query = new URLSearchParams({
        manifest_id: request.workloadId,
        manifest_version: request.manifestVersion,
        profile_id: request.profileId,
        profile_type: request.scope.profileType,
        resolved_profile_digest: request.scope.resolvedProfileDigest,
        draft_id: request.sourceDraft.draftId,
        expected_revision: String(request.sourceDraft.revision),
        expected_digest: request.sourceDraft.manifestDigest,
        proposal_set_digest: request.proposalSetDigest,
      ...
      request.proposalIds.forEach((proposalId) => query.append('proposal_ids', proposalId))
      const response = await requestJson(
        `/v1/cohort-proposals/decisions?${query.toString()}`,
        { method: 'GET' },
      )
    ...
    submitDecision: async (request: CohortDecisionSubmitRequest) => {
      ...
      const decision = parseCohortDecisionRecord(
        await requestJson(
      ...
      if (
        ...
        decision.candidateId !== (request.candidate?.candidateId ?? null)
      ) {
        throw new Error('Cohort API returned a decision outside the submitted request.')
      }
      return decision
    },
  }
